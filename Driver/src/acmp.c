...
void ACMP_EnableEventOut( uint32_t ACMPx)
{
	...
	if( ACMP0 == ACMPx )
	{
		ACMP->CVECON |=  ACMP_CEVCON_EVE0_Msk;		
	}
	if( ACMP1 == ACMPx )
	{
		ACMP->CVECON |=  ACMP_CEVCON_EVE1_Msk ;	
	}		
	ACMP->LOCK = 0x00;	
}
...
void ACMP_ConfigEventAndIntMode( uint32_t ACMPx, uint32_t EventAndIntMode)
{
	...
	if( ACMP0 == ACMPx )
	{
		ACMP->CVECON &= ~(ACMP_CEVCON_EVS0_Msk);
		ACMP->CVECON |=  EventAndIntMode;		
	}
	if( ACMP1 == ACMPx )
	{
		ACMP->CVECON &= ~(ACMP_CEVCON_EVS1_Msk);
		ACMP->CVECON |= (EventAndIntMode<< ACMP_CEVCON_EVS1_Pos);		
	}		
	ACMP->LOCK = 0x00;	
}
 ...
 ** \param [in] ACMPx: ACMP0 ,ACMP1
 ** \param [in] IntMod
 **       		ACMP_INT_Compx_Event：事件中断
 ** 					ACMP_INT_Compx_OutHigh：输出高中断
 ** 					ACMP_INT_Compx_OutLow：输出低中断
 ** \return  none
 ** \note   
 *****************************************************************************/
void ACMP_EnableInt(uint32_t ACMPx,uint8_t IntMod)
{
	...
	if(ACMPx == ACMP0)
		...
		ACMP->IMSC |= 1 << (ACMP_IMSC_C1IF_Pos + IntMod);
...
{
	if(ACMPx == ACMP0)
		return((ACMP->MIS &(0x1<<IntMod))? 1:0);
	else
		return((ACMP->MIS &(0x1<<(IntMod +ACMP_IMSC_C1IF_Pos)))? 1:0);
...
void ACMP_ClearIntFlag( uint32_t ACMPx,uint8_t IntMod)
{
	...
	if(ACMPx == ACMP0)
		ACMP->ICLR |=(0x1<<IntMod);
	else
		ACMP->ICLR |=(0x1<<(IntMod + ACMP_IMSC_C1IF_Pos));
}
