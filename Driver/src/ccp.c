 ...
 **					   (3) CCP_CLK_DIV_16
 **					   (4) CCP_CLK_DIV_64
 **		ReloadMode: CCP0 (1) CCP_RELOAD_0XFFFF    计数器重加载值为0xFFFF (连续模式)
...
void CCP_ConfigCompare(uint32_t CCPn, uint32_t CCPChannel, uint32_t Compare)
{
	...
	if(CCP0 == CCPn)
	{
		...
		{
			CCPTemp = CCP->D0A;
			CCPTemp &= ~(CCP_CCPD0A_DATA_Msk); 
			CCPTemp |= (uint16_t)Compare;
			CCP->D0A = CCPTemp;
		}
		if( CCPxB == CCPChannel)
		{
			CCPTemp = CCP->D0B;
			CCPTemp &= ~(CCP_CCPD0B_DATA_Msk); 
			CCPTemp |= (uint16_t)Compare;
			CCP->D0B = CCPTemp;
		}	
	}
...
void CCP_EnableReverseOutput(uint32_t CCPn, uint32_t CCPChannel)
{
	...
	if(CCP1 == CCPn)
	{
		if( CCPxA == CCPChannel)
		{
			CCP->CON1 |= CCP_CCPCON1_CCP1PWM1AOP_Msk;
		}
		if( CCPxB == CCPChannel)
		{
			CCP->CON1 |= CCP_CCPCON1_CCP1PWM1BOP_Msk;
		}
	}	
	CCP->LOCK = 0x00;	
}
 ...
 **			写寄存器触发捕获模式1、模式2捕获功能
 ** \param [in] CAPn ：CAP0(仅捕获模式1) 、CAP1、CAP2 、CAP3
 ** \return  none
 ** \note	 
*****************************************************************************/
void CCP_TriggerCaputer(uint32_t CAPn)
{
	CCP->LOCK = CCP_P1A_WRITE_KEY;
	if(CAPn == CAP0)
		CCP->CAPCON2 |= 0x55AA << 16;
	else if(CAPn == CAP1)
		CCP->CAPCON2 |= 0x55AA;
	else if(CAPn == CAP2)
		CCP->CAPCON3 |= 0x55AA << 16;
	else 
		CCP->CAPCON3 |= 0x55AA;
	CCP->LOCK = 0x00;
