 ...
 ** \param [in] EPWMn : EPWM0 、EPWM1 、EPWM2 、EPWM3 、EPWM4 、EPWM5、EPWM6
 **                    、EPWM7 	 
...
{
	EPWM->LOCK = EPWM_LOCK_P1B_WRITE_KEY;	
	EPWM->CLKPSC = (0x01<<EPWM_CLKPSC_CLKPSC67_Pos |
					0x01<<EPWM_CLKPSC_CLKPSC45_Pos |
...
void EPWM_ConfigChannelSymDuty(uint32_t EPWMn,uint32_t Duty)
{
	uint32_t PWMTemp=0;
	EPWM->LOCK = EPWM_LOCK_P1B_WRITE_KEY;	
	PWMTemp = EPWM->CMPDAT[EPWMn];
	PWMTemp &= 0xffff0000;
	PWMTemp |= 0xffff & Duty;
	EPWM->CMPDAT[EPWMn] = PWMTemp;
 ...
 **							EPWM_CH_3_MSK 、EPWM_CH_4_MSK 、EPWM_CH_5_MSK、EPWM_CH_6_MSK  
 **							 EPWM_CH_7_MSK  	 
 ** \return none
 ** \note   
*****************************************************************************/
...
{
	EPWM->LOCK = EPWM_LOCK_P1B_WRITE_KEY;	
	EPWM->CON3 |= (0x01UL<<(Event + EPWM_CON4_LETGACMP0_Pos));
	EPWM->LOCK = 0x0;
}
 ...
 **							EPWM_CH_3_MSK 、EPWM_CH_4_MSK 、EPWM_CH_5_MSK、EPWM_CH_6_MSK  
 **							 EPWM_CH_7_MSK  	 
 **				DeadZone : 0x00 ~ 0xFFFF;
 ** \return none
 ** \note   
*****************************************************************************/
void EPWM_EnableDeadZone(uint32_t EPWMnMask, uint32_t DeadZone,uint8_t DeadDir)
{
	...
	if(( EPWMnMask & EPWM_CH_0_MSK) || (EPWMnMask & EPWM_CH_1_MSK))
	{
		...
		if(DeadDir == EPWM_DTCTL_DIRRIGHT)
		{
			EPWM->DTCTL01 &= ~(EPWM_DTCTL_DTI01R_Msk);
			EPWM->DTCTL01 |= DeadZone << EPWM_DTCTL_DTI01R_Pos;
		}
		if(DeadDir == EPWM_DTCTL_DIRLEFT)
		{
			EPWM->DTCTL01 &= ~(EPWM_DTCTL_DTI01L_Msk);
			EPWM->DTCTL01 |= DeadZone << EPWM_DTCTL_DTI01L_Pos;
		}
	}
	if(( EPWMnMask & EPWM_CH_2_MSK) || (EPWMnMask & EPWM_CH_3_MSK))
	{
		...
		if(DeadDir == EPWM_DTCTL_DIRRIGHT)
		{
			EPWM->DTCTL23 &= ~(EPWM_DTCTL_DTI01R_Msk);
			EPWM->DTCTL23 |= DeadZone << EPWM_DTCTL_DTI01R_Pos;
		}
		if(DeadDir == EPWM_DTCTL_DIRLEFT)
		{
			EPWM->DTCTL23 &= ~(EPWM_DTCTL_DTI01L_Msk);
			EPWM->DTCTL23 |= DeadZone << EPWM_DTCTL_DTI01L_Pos;
		}
	}
	if(( EPWMnMask & EPWM_CH_4_MSK) || (EPWMnMask & EPWM_CH_5_MSK))
	{
		...
		if(DeadDir == EPWM_DTCTL_DIRRIGHT)
		{
			EPWM->DTCTL45 &= ~(EPWM_DTCTL_DTI01R_Msk);
			EPWM->DTCTL45 |= DeadZone << EPWM_DTCTL_DTI01R_Pos;
		}
		if(DeadDir == EPWM_DTCTL_DIRLEFT)
		{
			EPWM->DTCTL45 &= ~(EPWM_DTCTL_DTI01L_Msk);
			EPWM->DTCTL45 |= DeadZone << EPWM_DTCTL_DTI01L_Pos;
		}
	}
	if(( EPWMnMask & EPWM_CH_6_MSK) || (EPWMnMask & EPWM_CH_7_MSK))
	{
		EPWM->CON |= EPWM_CON_ENDT67_Msk;
		if(DeadDir == EPWM_DTCTL_DIRRIGHT)
		{
			EPWM->DTCTL67 &= ~(EPWM_DTCTL_DTI01R_Msk);
			EPWM->DTCTL67 |= DeadZone << EPWM_DTCTL_DTI01R_Pos;
		}
		if(DeadDir == EPWM_DTCTL_DIRLEFT)
		{
			EPWM->DTCTL67 &= ~(EPWM_DTCTL_DTI01L_Msk);
			EPWM->DTCTL67 |= DeadZone << EPWM_DTCTL_DTI01L_Pos;
		}
	}
 ...
 **					  	  (6)EPWM_MASK_LOAD_EPWM6	在EPWM6 的加载点加载掩码数据
 **					  	  (7)EPWM_MASK_LOAD_EPWM7	在EPWM7 的加载点加载掩码数据
 ** \return none
...
{
	EPWM->LOCK = EPWM_LOCK_P1B_WRITE_KEY;	
	EPWM->IMSC2 |= EPWM_IMSC2_ENBRKIF_Msk;
	EPWM->LOCK = 0x0;	
}
/*****************************************************************************
...
{
	return ((EPWM->MIS2&EPWM_MIS2_BRKIF_Msk)? 1:0);
}
/*****************************************************************************
...
{
	EPWM->ICLR2 |= EPWM_ICLR2_BRKIF_Msk;
}
...
{
	EPWM->LOCK = EPWM_LOCK_P1B_WRITE_KEY;	
	EPWM->IMSC2 |= EPWM_IMSC2_ENHALLIF_Msk;
	EPWM->LOCK = 0x0;		
}
/*****************************************************************************
...
{
	return ((EPWM->MIS2&EPWM_MIS2_HALLIF_Msk)? 1:0);
}
/*****************************************************************************
...
{
	EPWM->ICLR2 |= (EPWM_ICLR2_HALLIF_Msk);
}
...
{
	return((EPWM->MIS2 & (0x1<<EPWMCmp))? 1:0);
}
/*****************************************************************************
...
{
	EPWM->LOCK = EPWM_LOCK_P1B_WRITE_KEY;	
	EPWM->CON3 &= ~(EPWM_CON4_LOADNWINT_Msk);
	EPWM->LOCK = 0x0;			
}
/*****************************************************************************
